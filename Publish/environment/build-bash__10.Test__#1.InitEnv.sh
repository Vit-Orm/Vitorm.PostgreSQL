...
docker run -t --rm --link vitorm-postgres postgres:15.8 timeout 120 sh -c "export PGPASSWORD=123456; until psql -h postgres -U postgres -d db_orm -c 'create database db_orm2;'; do echo waiting for PostgreSQL; sleep 2; done;"
